                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="badge bg-secondary">{{ number_format($test->total_questions) }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-dark">{{ $test->total_columns }} × {{ $test->rows_per_column }}</span>
                                </td>
                                <td class="text-center">
                                    @if($test->is_active)
                                ...
                                <td class="text-center">
                                    <span class="badge bg-info">{{ $test->test_sessions_count ?? 0 }}</span>
                                </td>
                                <td class="text-center">
...
<script>
    ...
    function deleteTest(testId, testName) {
        Swal.fire({
            title: 'Delete Test?',
            html: `Are you sure you want to delete <strong>${testName}</strong>?<br><span class="text-danger">This action cannot be undone!</span>`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                fetch(`/tester/tests/${testId}`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire('Deleted!', 'Test has been deleted.', 'success').then(() => location.reload());
                        } else {
                            Swal.fire('Error!', 'Failed to delete test', 'error');
                        }
                    }).catch(() => Swal.fire('Error!', 'Error deleting test', 'error'));
            }
        });
    }
</script>
